...
data class StandardExercise(
    ...
    val tip: String? = null,
    val verbs: List<String> = emptyList(), // Нові поля для collocation_sort
    val nouns: List<String> = emptyList(),
    val options: List<String> = emptyList() // Опції для matching
)

@Serializable
data class Block2Grammar(
    @SerialName("grammar_topic") val grammarTopic: String = "",
    @SerialName("explanation_ua") val explanationUa: String = "", // В JSON explanation_ua
    val rules: List<GrammarRule> = emptyList(),
    @SerialName("forms_table") val formsTable: FormsTable? = null, // Єдина таблиця з формами
...
// Оскільки reading тепер об'єкт з іменованими вправами, треба адаптувати DTO
@Serializable
...
data class ReadingExercise(
    ...
    val instruction: String = "",
    val text: String? = null,
    val texts: Map<String, String>? = null, // Для advertisement (A, B, C)
    val items: List<String> = emptyList(),
    val answers: List<String> = emptyList()
)

// Listening також об'єкт з іменованими вправами
@Serializable
...
data class ListeningExercise(
    ...
    val answers: List<String> = emptyList() // Для exercise_1
)
